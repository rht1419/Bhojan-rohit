...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  String _tenantLabel = 'Loading tenant...';
  ...
  Future<void> _resolveTenantLabel() async {
    final authState = ref.read(authNotifierProvider);
    final tenantId = authState.user?.tenantId;
    if (tenantId == null || tenantId.isEmpty) {
      if (mounted) {
        setState(() => _tenantLabel = 'Guest');
      }
      return;
    }
    ...
    try {
      ...
      final tenants = await authService.getTenants();
      final tenant = tenants.cast<dynamic>().firstWhere(
        (t) => t != null && t.id == tenantId,
        orElse: () => null,
      );

      if (mounted) {
        setState(() => _tenantLabel = tenant?.name ?? tenantId);
      }
    } catch (_) {
      if (mounted) {
        setState(() => _tenantLabel = tenantId);
      }
    }
  }
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Text(_tenantLabel, style: AppTextStyles.labelLarge.copyWith(fontSize: 18)),
